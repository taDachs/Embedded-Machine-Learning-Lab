...
def display_result(
    ...
    if output:
        ...
        for i in range(bboxes.shape[1]):
            ...
            if bboxes[0, i, -1] >= 0:
                cx = (
                    int(bboxes[0, i, 0] * img_shape - bboxes[0, i, 2] * img_shape / 2)
                    + pad
                )
                cy = (
                    int(bboxes[0, i, 1] * img_shape - bboxes[0, i, 3] * img_shape / 2)
                    + pad
                )
                ...
                rect = patches.Rectangle(
                    (cx, cy), w, h, linewidth=2, facecolor="none", edgecolor="r"
                )
                ax.add_patch(rect)
                ax.annotate(
                    num_to_class(int(bboxes[0, i, 5]))
                    + " "
                    + f"{float(bboxes[0,i,4]):.2f}",
                    (cx, cy),
                    color="r",
                )
    ...
    for i in range(target.shape[1]):
        if target[0, i, -1] >= 0:
            cx = (
                int(target[0, i, 0] * img_shape - target[0, i, 2] * img_shape / 2) + pad
            )
            cy = (
                int(target[0, i, 1] * img_shape - target[0, i, 3] * img_shape / 2) + pad
            )
            ...
            rect = patches.Rectangle(
                (cx, cy), w, h, linewidth=2, facecolor="none", edgecolor="g"
            )
            ax.add_patch(rect)
